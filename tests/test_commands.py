...
class TestExtractHexDigest:
    def test_extracts_sha512_from_rhash_output(self):
        text = "SHA512  file.txt\n" + "a" * 128 + "  file.txt\n"
        assert _extract_hex_digest(text) == "a" * 128

    def test_extracts_blake3_from_hash_file(self):
...
class TestAgeEncryptFile:
    ...
    def test_passphrase_success(self, mock_spawn):
        mock_child = MagicMock()
        mock_child.exitstatus = 0
        mock_child.before = b""
        mock_spawn.return_value = mock_child

        ok, msg = age_encrypt_file(
            Path("/tmp/test.txt"), passphrase="secret123"
        )
        assert ok is True
    ...
    def test_recipient_success(self, mock_run_cmd):
        mock_run_cmd.return_value = (0, "", "")
        ok, msg = age_encrypt_file(
            Path("/tmp/test.txt"), recipient="age1abc..."
        )
        assert ok is True
        assert "Encrypted" in msg

    def test_no_passphrase_no_recipient(self):
    ...
    def test_age_not_found(self, mock_spawn):
        ok, msg = age_encrypt_file(
            Path("/tmp/test.txt"), passphrase="secret"
        )
        assert ok is False
...
class TestAgeDecryptFile:
    ...
    def test_passphrase_success(self, mock_spawn):
        ...
        ok, msg = age_decrypt_file(
            Path("/tmp/test.txt.age"), passphrase="secret123"
        )
        assert ok is True
        assert "Decrypted" in msg

    @patch("mycryptopony.run_cmd")
    def test_identity_success(self, mock_run_cmd):
        mock_run_cmd.return_value = (0, "", "")
        ok, msg = age_decrypt_file(
            Path("/tmp/test.txt.age"), identity=Path("/tmp/key.txt")
        )
        assert ok is True
        assert "Decrypted" in msg

    def test_no_passphrase_no_identity(self):
...
class TestMinisignSignFile:
    @patch("mycryptopony.pexpect.spawn")
    def test_success(self, mock_spawn):
        mock_child = MagicMock()
        mock_child.exitstatus = 0
        mock_child.before = b""
        mock_spawn.return_value = mock_child
    ...
    @patch("mycryptopony.pexpect.spawn", side_effect=FileNotFoundError)
    def test_minisign_not_found(self, mock_spawn):
        ok, msg = minisign_sign_file(
            Path("/tmp/test.txt"), None, None, "pass"
        )
        assert ok is False
        assert "not found" in msg
...
class TestMinisignVerifyFile:
    @patch("mycryptopony.run_cmd")
    def test_valid_signature(self, mock_run_cmd):
        mock_run_cmd.return_value = (0, "", "")
        ok, msg = minisign_verify_file(
            Path("/tmp/test.txt"),
            Path("/tmp/test.txt.minisig"),
            pub_key_path=None,
        )
        assert ok is True
        assert "valid" in msg
    ...
    @patch("mycryptopony.run_cmd")
    def test_invalid_signature(self, mock_run_cmd):
        mock_run_cmd.return_value = (1, "", "signature verification failed")
        ok, msg = minisign_verify_file(
            Path("/tmp/test.txt"),
            Path("/tmp/test.txt.minisig"),
            pub_key_path="/tmp/pub.key",
        )
        assert ok is False
...
# ---------------------------------------------------------------------------
# rhash_verify_hash tests (the fixed function)
# ---------------------------------------------------------------------------

class TestRhashVerifyHash:
    @patch("mycryptopony.run_cmd")
    def test_hash_matches(self, mock_run_cmd, tmp_path):
        # Create a hash file with a known hash
        expected_hash = "a" * 128
        hash_file = tmp_path / "file.txt.sha512"
        hash_file.write_text(f"{expected_hash}  file.txt\n")

        # Mock rhash to return the same hash for the target file
        mock_run_cmd.return_value = (
            0,
            f"{expected_hash}  file.txt\n",
            "",
        )
    ...
    @patch("mycryptopony.run_cmd")
    def test_hash_mismatch(self, mock_run_cmd, tmp_path):
        hash_file = tmp_path / "file.txt.sha512"
        hash_file.write_text("a" * 128 + "  file.txt\n")

        # Mock rhash to return a DIFFERENT hash
    ...
    @patch("mycryptopony.run_cmd")
    def test_target_file_actually_used(self, mock_run_cmd, tmp_path):
        ...
        # Verify that rhash was called with the TARGET file, not the hash file
        cmd = mock_run_cmd.call_args[0][0]
        assert str(target_file) in cmd
        assert str(hash_file) not in cmd

    @patch("mycryptopony.run_cmd")
    def test_rhash_failure(self, mock_run_cmd, tmp_path):
        hash_file = tmp_path / "file.txt.sha512"
        hash_file.write_text("a" * 128 + "  file.txt\n")
    ...
    @patch("mycryptopony.run_cmd")
    def test_unreadable_hash_file(self, mock_run_cmd, tmp_path):
...
class TestCrocSend:
    @patch("mycryptopony.run_cmd")
    def test_success_with_code(self, mock_run_cmd):
        mock_run_cmd.return_value = (
            0,
            "Code is: puma-moral-builder\n",
            "Sending...\n",
        )
    ...
    @patch("mycryptopony.run_cmd")
    def test_success_without_code(self, mock_run_cmd):
